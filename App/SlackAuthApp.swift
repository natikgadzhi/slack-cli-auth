...
/// A single-window macOS app: open it, sign in to Slack, and the captured tokens
/// land in the Keychain for slack-cli. Quits when its window closes.
@main
struct SlackAuthApp: App {
  ...
  var body: some Scene {
    Window("Slack CLI Helper", id: "login") {
    ...
    .commands {
      CommandGroup(replacing: .appInfo) {
        Button("About Slack CLI Helper", action: About.showPanel)
      }
    }
  }
}
